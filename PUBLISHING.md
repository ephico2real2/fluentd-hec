...
This is a **fork** of the archived `splunk/fluentd-hec`. Upstream is read-only and the image it used to
publish, `splunk/fluentd-hec:1.3.3`, **has been deleted from Docker Hub** — a cluster that loses its cached
...
```sh
gh secret list --repo "ephico2real2/fluentd-hec"
```
...
```
docker.io/ephico2real/fluentd-hec:1.3.3-h1-g<sha>     immutable — pin THIS in a cluster
docker.io/ephico2real/fluentd-hec:1.3.3-h1            rolling within the hardening pass
quay.io/ephico2real/fluentd-hec:1.3.3-h1-g<sha>       mirror, same digest
quay.io/ephico2real/fluentd-hec:1.3.3-h1              mirror, same digest
```
...
```sh
gh workflow run publish-image.yaml --repo ephico2real2/fluentd-hec \
...
```sh
git tag image-v1.3.3-h1 && git push origin image-v1.3.3-h1
```
...
```
ephico2real/fluentd-hec:1.3.3-h1-g<sha>   # immutable — pin THIS in a cluster
ephico2real/fluentd-hec:1.3.3-h1          # rolling within the hardening pass
```

`1.3.3` is the upstream release this forked from (the `VERSION` file). `h1` is the hardening pass — bump
...
In `splunk-connect-for-kubernetes` `values.yaml`, under `splunk-kubernetes-logging`:

```yaml
splunk-kubernetes-logging:
  image:
    registry: docker.io
    name: ephico2real/fluentd-hec
    tag: 1.3.3-h1-g<sha>
```
