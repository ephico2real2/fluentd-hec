# Adoption record — fluent-plugin-splunk-hec

Upstream `splunk/fluent-plugin-splunk-hec` was archived by Splunk on 2025-06-24 (End of Support was
...
|---|---|---|---|
| h2 (default) | `ephico2real/fluentd-hec:1.3.3-h2` | hardened base + jq + minimal shell | `develop` / `main` |
| h1 (frozen) | `ephico2real/fluentd-hec:1.3.3-h1` | hardened base, no shell at all | `feat/jq-free` |
...
```
docker.io/ephico2real/fluentd-hec:1.3.3-h2      # rolling — jq-capable (jq 1.8.2 + minimal shell)
docker.io/ephico2real/fluentd-hec:1.3.3-h2-g<sha>   # immutable — pin this in production
quay.io/ephico2real/fluentd-hec:1.3.3-h2      # mirror, same digest
```
